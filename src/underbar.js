(function() {
  ...
  _.last = function(array, n) {
    return n === undefined ? array[array.length - 1] : n < 1 ? [] : array.slice(-n);
  };
  ...
  _.each = function(collection, iterator) {
    if (Array.isArray(collection)) {
      for (var ind = 0; ind < collection.length; ind++) {
        iterator(collection[ind], ind, collection);
      }
    } else {
  ...
  _.defaults = function(obj) {
    _.each(arguments, function(element) {
      _.each(element, function(value, prop) {
        obj[prop] === undefined && (obj[prop] = value);
      });
    });
    return obj;
  };
  ...
  _.shuffle = function(array) {
    ...
    for (var index = 0; index < array.length; index++) {
      var rando = Math.floor(Math.random()*index);
      if (rando !== index) {
        copy[index] = copy[rando];
      }
      copy[rando] = array[index];
    }
    return copy;
  };
  ...
  _.invoke = function(collection, functionOrKey, args) {
    ...
    for (var index = 0; index < collection.length; index++) {
      result.push(functionOrKey.apply(collection[index], args));
    }
    return result;
  };
